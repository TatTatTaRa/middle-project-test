...
// ── 타입 팝오버 ──
const btnTypePopover = document.getElementById('btnTypePopover');
const typePopover = document.getElementById('typeFilterPopover');
const typeFilterBtnText = document.getElementById('typeFilterBtnText');

btnTypePopover.addEventListener('click', (e) => {
  e.stopPropagation();
  const isOpen = typePopover.style.display === 'block';
  // 다른 팝오버 닫기
  capPopover.style.display = 'none';
  typePopover.style.display = isOpen ? 'none' : 'block';
});

// 타입 행 클릭 (커스텀 체크박스)
document.querySelectorAll('#filterTypeGroup .fp-cb-row').forEach(row => {
...
// 타입 초기화
document.getElementById('btnTypeReset').addEventListener('click', (e) => {
  e.stopPropagation();
  selectedTypes.clear();
  document.querySelectorAll('#filterTypeGroup .fp-cb-row').forEach(r => r.classList.remove('checked'));
  updateTypeBtnText();
  applyFilters();
});

function updateTypeBtnText() {
  if (selectedTypes.size === 0) {
    typeFilterBtnText.textContent = '전체';
    btnTypePopover.classList.remove('has-filter');
  } else {
    const labels = { STANDARD: 'Standard', DELUXE: 'Deluxe', SUITE: 'Suite' };
    const arr = Array.from(selectedTypes);
    const first = labels[arr[0]] || arr[0];
    typeFilterBtnText.textContent = arr.length === 1 ? first : `${first} 외 ${arr.length - 1}개`;
    btnTypePopover.classList.add('has-filter');
  }
}

// ── 가용 인원 팝오버 ──
const btnCapPopover = document.getElementById('btnCapPopover');
const capPopover = document.getElementById('capFilterPopover');
const capFilterBtnText = document.getElementById('capFilterBtnText');

btnCapPopover.addEventListener('click', (e) => {
  e.stopPropagation();
  const isOpen = capPopover.style.display === 'block';
  // 다른 팝오버 닫기
  typePopover.style.display = 'none';
  capPopover.style.display = isOpen ? 'none' : 'block';
});

document.querySelectorAll('#filterCapGroup .fp-cap-btn').forEach(btn => {
  btn.addEventListener('click', (e) => {
    ...
    if (selectedCap === cap) {
      ...
      selectedCap = cap;
      document.querySelectorAll('#filterCapGroup .fp-cap-btn').forEach(b => b.classList.remove('active'));
...
document.getElementById('btnCapReset').addEventListener('click', (e) => {
  e.stopPropagation();
  selectedCap = 'all';
  document.querySelectorAll('#filterCapGroup .fp-cap-btn').forEach(b => b.classList.remove('active'));
  updateCapBtnText();
...
function updateCapBtnText() {
  const personIcon = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:middle; margin-right:2px;"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>`;
  if (selectedCap === 'all') {
    capFilterBtnText.innerHTML = `${personIcon}전체`;
    btnCapPopover.classList.remove('has-filter');
  } else {
    const label = selectedCap === '6' ? '6인+' : `${selectedCap}인`;
    capFilterBtnText.innerHTML = `${personIcon}${label}`;
    btnCapPopover.classList.add('has-filter');
  }
}
...
typePopover.addEventListener('click', e => e.stopPropagation());
capPopover.addEventListener('click', e => e.stopPropagation());
...
function applyFilters() {
  ...
  document.querySelectorAll('.unassigned-card').forEach(card => {
    ...
    const cap = (type === 'DELUXE') ? 4 : (type === 'SUITE') ? 6 : 2;

    const okType = showAllTypes || selectedTypes.has(type);
    let okCap = true;
    if (selectedCap !== 'all') {
      const fv = parseInt(selectedCap);
      okCap = (fv === 6) ? cap >= 6 : cap === fv;
    }
  ...
  document.querySelectorAll('.tl-row').forEach(row => {
    ...
    row.style.display = (okType && okCap) ? 'contents' : 'none';
  });
}
